    ...
    for (var i=0; i < calendar.length; i++) {
        ...
        for (var j=0; j < semesterInfo.vacationDates.length; j++) {
            if ((date.getTime() >= semesterInfo.vacationDates[j].startDate.getTime()) &&
                ...
                calendar[i].lab = semesterInfo.vacationDates[j];
            }
        }
    }
}
...
function getTopicString(topic) {
    ...
    if (topic instanceof Topic) {
        ...
        str = "<strong>" + topic.section + " @ " + getStandardTimeString(topic.date) + "</strong>";
    }
    return str;
}
...
function printLabs(opts) {
    ...
    for (var i = 0; i < numItems; i++) {
		...
		if (!calendar[i].lab) {
         	// no lab on this date, so use N/A
            document.write("<td>" + "N/A" + "</td>");
            document.write("<td></td>");
        } else {
        	var labstr = getLabString(calendar[i].lab, calendar[i].date);
        	if (labstr === "") {
        		document.write("<td>" + getTopicString(calendar[i].topic) + "</td>");
        		document.write("<td></td>");
        	} else {
				document.write("<td>" + labstr + "</td>");
				document.write("<td>");
